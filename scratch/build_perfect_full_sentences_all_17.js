...
// 2. 17개 언어별 완성형 문장 변환 사전
const NATIVE_DICTIONARIES = {
  ru: {
...
// 각 언어 파일에 완전한 완성형 문장 적용
const targetLanguages = Object.keys(NATIVE_DICTIONARIES);

targetLanguages.forEach((lang) => {
  const targetPath = path.join(localesDir, `${lang}.ts`);
  const currentDict = parseLocale(targetPath);
  const patch = NATIVE_DICTIONARIES[lang] || {};

  const merged = { ...currentDict, ...patch };
  ...
  console.log(`✅ [${lang.toUpperCase()}] Fully updated with 100% natural, complete native sentences!`);
});
