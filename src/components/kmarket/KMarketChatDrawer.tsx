...
export default function KMarketChatDrawer() {
  ...
  const quickPhrases = [
    { label: t('👋 구매 문의'), text: t('안녕하세요! 아직 판매 중인가요?') },
    { label: t('💰 가격 네고'), text: t('조금만 깎아주실 수 있나요? (네고 가능한가요?)') },
    { label: t('📍 공단 직거래'), text: t('오늘 저녁 기숙사 앞이나 공단 정문에서 직거래 가능할까요?') },
    { label: t('🤝 거래 수락'), text: t('네, 좋습니다! 그때 뵐게요.') },
  ];
