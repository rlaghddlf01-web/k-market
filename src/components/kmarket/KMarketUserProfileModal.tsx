...
import KMarketUserItemsList from './KMarketUserItemsList';
import CountryFlag from './CountryFlag';
import { X, ShieldCheck, Home, MessageSquare, Award, ThumbsUp, ShoppingBag, UserCheck } from 'lucide-react';
...
export default function KMarketUserProfileModal({
  ...
  // 이 판매자가 올린 매물 개수 계산
  const sellerItemsCount = items.filter(
    (item) => item.seller_id === userId || item.seller_name?.includes(userName)
  ).length;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white dark:bg-gray-900 rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-gray-100 dark:border-gray-800 relative max-h-[90vh] overflow-y-auto flex flex-col">
        {/* 닫기 버튼 */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-full text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="space-y-5 flex-1">
          ...
          {/* 2단 탭 바 (🛍️ 판매 물품 vs ⭐ 매너 & 후기) */}
          <div className="flex rounded-2xl bg-slate-100 dark:bg-gray-800 p-1">
            <button
            ...
            >
              ...
              <span>판매 상품 ({sellerItemsCount})</span>
            </button>
