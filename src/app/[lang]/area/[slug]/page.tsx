...
// [K-Market SEO] 산업단지 근로자 직거래 랜딩 페이지
// Route: /[lang]/area/[slug]
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `[${title}] Free Secondhand Appliances & Moving Sale for Workers | K-Market Korea`,
    description: `Browse verified free listings near ${title} in Korea. 17-language AI chat. No scams. ${langInfo.name} supported.`,
    alternates: {
      canonical: `https://ktrs-market.vercel.app/${lang}/area/${slug}`,
      languages: Object.fromEntries(
        Object.keys(LANG_MAP).map(l => [l, `https://ktrs-market.vercel.app/${l}/area/${slug}`])
      ),
    },
    openGraph: {
      title: `[${title}] K-Market 🏭 Free Secondhand Appliances & Moving Sale for Workers`,
      description: `Verified K-Market listings near ${title}. ${langInfo.name} supported.`,
      url: `https://ktrs-market.vercel.app/${lang}/area/${slug}`,
      siteName: 'K-Market Korea',
    },
  };
}

export default async function KMarketAreaSEOPage({ params }: Props) {
  const { lang, slug } = await params;
  const langInfo = LANG_MAP[lang] || LANG_MAP['en'];
  const title = slugToTitle(slug);
  const cta = `https://ktrs-market.vercel.app/${lang}?utm_source=google_seo&utm_medium=organic&utm_campaign=area_${slug}`;

  const jsonLd = {
    '@context': 'https://schema.org/',
    '@type': 'LocalBusiness',
    name: `K-Market @ ${title}`,
    description: `Free Secondhand Appliances & Moving Sale for Workers near ${title} in Korea.`,
    url: `https://ktrs-market.vercel.app/${lang}/area/${slug}`,
      ...
      <div style={{ fontFamily: "'Inter', sans-serif", background: '#0f172a', minHeight: '100vh', color: '#f8fafc' }}>
        ...
        <section style={{ maxWidth: '900px', margin: '0 auto', padding: '60px 24px 40px', textAlign: 'center' }}>
          ...
          <h1 style={{ fontSize: '30px', fontWeight: 900, lineHeight: 1.3, marginBottom: '16px' }}>
            {title} — Free Secondhand Appliances & Moving Sale for Workers
          </h1>
          <p style={{ color: '#94a3b8', fontSize: '16px', lineHeight: 1.8, marginBottom: '32px' }}>
            {langInfo.welcome}! 산업단지 근로자 직거래 · {langInfo.flag} {langInfo.name}
          </p>
