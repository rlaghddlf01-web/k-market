...
// [K-Market SEO] 외국인 밀집촌 커뮤니티 랜딩 페이지
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `[${title}] Expat Community Marketplace & 0 KRW Free Share | K-Market Korea`,
    description: `Browse verified free listings near ${title} in Korea. 17-language AI chat. No scams. ${langInfo.name} supported.`,
    alternates: {
      canonical: `https://ktrs-market.vercel.app/${lang}/town/${slug}`,
      languages: Object.fromEntries(
        Object.keys(LANG_MAP).map(l => [l, `https://ktrs-market.vercel.app/${l}/town/${slug}`])
      ),
    },
    openGraph: {
      title: `[${title}] K-Market 🌏 Expat Community Marketplace & 0 KRW Free Share`,
      description: `Verified K-Market listings near ${title}. ${langInfo.name} supported.`,
      url: `https://ktrs-market.vercel.app/${lang}/town/${slug}`,
      siteName: 'K-Market Korea',
    },
  };
}

export default async function KMarketTownSEOPage({ params }: Props) {
  const { lang, slug } = await params;
  const langInfo = LANG_MAP[lang] || LANG_MAP['en'];
  const title = slugToTitle(slug);
  const cta = `https://ktrs-market.vercel.app/${lang}?utm_source=google_seo&utm_medium=organic&utm_campaign=town_${slug}`;

  const jsonLd = {
    '@context': 'https://schema.org/',
    '@type': 'LocalBusiness',
    name: `K-Market @ ${title}`,
    description: `Expat Community Marketplace & 0 KRW Free Share near ${title} in Korea.`,
    url: `https://ktrs-market.vercel.app/${lang}/town/${slug}`,
      ...
      <div style={{ fontFamily: "'Inter', sans-serif", background: '#0f172a', minHeight: '100vh', color: '#f8fafc' }}>
        ...
        <section style={{ maxWidth: '900px', margin: '0 auto', padding: '60px 24px 40px', textAlign: 'center' }}>
          ...
          <h1 style={{ fontSize: '30px', fontWeight: 900, lineHeight: 1.3, marginBottom: '16px' }}>
            {title} — Expat Community Marketplace & 0 KRW Free Share
          </h1>
          <p style={{ color: '#94a3b8', fontSize: '16px', lineHeight: 1.8, marginBottom: '32px' }}>
            {langInfo.welcome}! 외국인 밀집촌 커뮤니티 · {langInfo.flag} {langInfo.name}
          </p>
